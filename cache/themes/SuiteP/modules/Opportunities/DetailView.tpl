...
{if !$fields.opportunities_documents_c.hidden}
{counter name="panelFieldCount" print=false}

{if !empty($fields.document_id_c.value)}
{capture assign="detail_url"}index.php?module=Documents&action=DetailView&record={$fields.document_id_c.value}{/capture}
<a href="{sugar_ajax_url url=$detail_url}">{/if}
<span id="document_id_c" class="sugar_field" data-id-value="{$fields.document_id_c.value}">{$fields.opportunities_documents_c.value}</span>
{if !empty($fields.document_id_c.value)}</a>{/if}
{/if}
